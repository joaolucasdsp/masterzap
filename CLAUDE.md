...
- **Base path**: the site can live under a prefix (GitHub Pages: `/<repo>/`). `SITE_URL` at build time sets it — `src/lib/base.js` derives `BASE` from its pathname, Vite gets it as `base`, and `scripts/lib/corpus.mjs` uses it as `SITE` for canonical URLs. **Never write a root-relative path as a literal** (`'/assets/x.png'`, `fetch('/data/…')`): use `withBase('/assets/x.png')` from `src/lib/base.js`. HTML the scripts generate goes through `rebaseHtml()` (prerender's `writeHtml`), which also re-points `https://www.masterwhats.com.br` at `SITE`; `index.html` gets the same in the `siteHtml` plugin in `vite.config.js`, which also drops the Vercel Insights script unless Vercel is building. Unit tests that look at `dist/` or at URLs import `SITE`/`BASE` instead of literals, so `SITE_URL=… npm test` checks a Pages build
